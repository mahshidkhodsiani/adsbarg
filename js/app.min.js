...
document.addEventListener("DOMContentLoaded", function () {
  const flipContainer = document.getElementById("flip-banner-container");
  const intervalTime = 4000; // ۵ ثانیه (می‌توانید تغییر دهید)

  if (flipContainer) {
    function flipCard() {
      // با اضافه/حذف کردن کلاس is-flipped، انیمیشن CSS فعال می‌شود
      flipContainer.classList.toggle("is-flipped");
    }

    // هر ۵ ثانیه تابع flipCard را اجرا کن
    setInterval(flipCard, intervalTime);
  }
});
...
document.addEventListener("DOMContentLoaded", function () {
  const flipContainer = document.getElementById("flip-banner-container");
  const intervalTime = 4000; // 4 ثانیه برای هر تصویر
  let currentState = 1;
  const totalStates = 3;
  ...
  if (flipContainer) {
    function rotateBanner() {
      // حذف کلاس‌های قبلی
      flipContainer.classList.remove("state-1", "state-2", "state-3");

      // اضافه کردن کلاس جدید
      flipContainer.classList.add(`state-${currentState}`);

      // افزایش شمارنده با حلقه
